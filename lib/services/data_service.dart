...
class DataService extends ChangeNotifier {
  ...
  List<SalarySegment> _buildSegments(
    ...
    for (int i = 0; i < configs.length; i++) {
      ...
      int directAmount = 0; // fixed amounts paid as-is (not via daily rate)
        ...
        if (r.date.compareTo(segFrom) >= 0 && r.date.compareTo(segTo) <= 0) {
          if (r.status == 'A') {
            ...
            if (r.cancelledDuty == 'all') {
              // Both shifts cancelled → no pay, count as absent
              absentDays++;

            } else if (r.cancelledDuty == 'day' || r.cancelledDuty == 'night') {
              // One shift cancelled → pay only cancelledShiftAmount (fixed, not daily rate)
              directAmount += r.cancelledShiftAmount;
              // Do NOT add to presentDays/doubleDutyDays — daily rate not used

            } else {
              // Full double duty — no cancellation
              if (cfg.dutyType == 'double') {
                // DD guard doing full PP → 1 full work day
                doubleDutyDays++;
              } else {
                // SD guard doing overtime PP → 1 work day + overtime on top
                doubleDutyDays++;
            ...
            if (r.cancelledDuty == 'all') {
              // Single duty cancelled → no pay, count as absent
              absentDays++;

            } else if (cfg.dutyType == 'double') {
              // DD guard doing only single duty → pay fixed singleDutyAmount only
              directAmount += r.singleDutyAmount;
              // Do NOT add to presentDays — daily rate not used

            } else {
              // SD guard doing their normal single duty
              presentDays++;
            }
          }
        }
      ...
      // segTotal = daily-rate pay + direct fixed amounts + overtime (SD guard PP)
      final segTotal = basePay + directAmount + overtimeAmount;
