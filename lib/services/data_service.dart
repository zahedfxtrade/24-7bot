      ...
      for (final r in records) {
        if (r.date.compareTo(segFrom) >= 0 && r.date.compareTo(segTo) <= 0) {
          if (r.status == 'A') {
            ...
            // If full duty is cancelled, skip entirely
            if (r.cancelledDuty == 'all') continue;
            if (r.cancelledDuty == null) {
              // No cancellation — full double duty
              doubleDutyDays++;
              if (cfg.dutyType != 'double') overtimeAmount += r.overtimeAmount;
            } else {
              // One shift (day or night) cancelled → counts as single present
              presentDays++;
            }
          } else if (r.status == 'P') {
            // If the single duty is cancelled, skip entirely
            if (r.cancelledDuty == 'all') continue;
            presentDays++;
