...
class _AttendanceScreenState extends State<AttendanceScreen> {
  ...
  Future<void> _cancelDuty(Guard g, AttendanceRecord rec) async {
    if (rec.status == 'PP') {
      ...
      if (choice != 'all') {
        // One shift cancelled — ask how much to pay for the remaining shift
        final amount = await _askRemainingShiftAmount(g, choice);
        if (amount == null) return;
        rec.cancelledShiftAmount = amount;
      } else {
        rec.cancelledShiftAmount = 0;
      }
  ...
  Future<int?> _askRemainingShiftAmount(Guard g, String cancelledShift) async {
    final remainingShift = cancelledShift == 'day' ? 'Night' : 'Day';
    final amtCtrl = TextEditingController();
    return showDialog<int>(
      context: context,
      builder: (_) => AlertDialog(
        title: Text('${g.name} – $remainingShift Duty Amount'),
        content: Column(mainAxisSize: MainAxisSize.min, children: [
          Text(
            '$remainingShift shift is present.\nHow much to pay for today\'s $remainingShift duty?',
          ...
          TextField(
            ...
            decoration: InputDecoration(
              labelText: '$remainingShift Duty Amount (₹)',
              prefixText: '₹ ',
            ),
          ),
        ]),
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.pop(context, int.tryParse(amtCtrl.text.trim()) ?? 0);
            },
